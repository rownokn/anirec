from service import AnimeInfo, AnimeBrowseInfo, AnimeNotFoundError,CharacterNotFoundError
...
def anime_profile(id):
    try:
        anime_data = AnimeInfo.anime_profile(id)
...
def anime_popular():
    ...
    return jsonify({"popular": pop_anime_data})


@app.route('/trend', methods=['GET'])
def anime_trend():
    trend_anime_data = AnimeBrowseInfo.trending_anime()
    return jsonify({"trend": trend_anime_data})
