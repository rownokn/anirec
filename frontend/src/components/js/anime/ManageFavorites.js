...
const ManageFavorites = ({account, anime_id, exist, setExist}) => {
  ...
  const addToFavorites = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/user/add_favorite', config);
  ...
  const deleteFavorites = async () => {
    try {
      const config = {
        ...
        const response = await fetch(`http://localhost:8000/user/delete_favorite/${account.account_id}/${anime_id}`, config);
        const userInfo = await response.json();
        setExist(userInfo.exist)
        toast.success(userInfo.msg, {
          position: toast.POSITION.TOP_CENTER,
